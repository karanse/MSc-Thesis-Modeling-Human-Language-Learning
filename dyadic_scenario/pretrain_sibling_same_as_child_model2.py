...
#                bbox_indices.append(ha_vggs_indices[img][obj][0])  # buggy script
                if obj_id == obj:
                    targets.append(n)
                n += 1
...
#            language_times = [] # All word indices in the batch?
#            visual_times = [] # All vgg vectors in the batch?
#            target_times = [] # All target word indices in the batch?
            for i in (0,1):
                language_input, visual_input, target = load_select_obj(dict_words_boxes, ha_vggs_indices, img,
                setting)
#                language_times.append(language_input)
#                visual_times.append(visual_input)
#                target_times.append(target)
        ...
        batch_size[batch] = 2*len(batches[batch])

        listener_loss = criterion(obj_guesses, targets)
        listener_loss.backward() # backward pass
        listener_optimizer.step() # adapting the weights

        # Loss/accuracy times batch size for weighted average over epoch:
        li_train_loss[batch] = listener_loss.item() * batch_size[batch]
    ...
    print('| S.loss {:8.4f} | S.acc. {:8.4f} |'.format(
        ...
        avg_sp_train_acc))
    print('-' * 89)
#    print("pretrained sibling listener model: \n\n", listener, '\n')
#    print("pretrained sibling speaker model: \n\n", speaker, '\n')
#    print("The state dict keys listener: \n\n", listener.state_dict().keys())
#    print("The state dict keys speaker: \n\n", listener.state_dict().keys())
...
epochs = int(sys.argv[3]) # start with 1 while testing
...
np.save('loss_acc_older/li_train_loss_{}_{}'.format(str(batchsize),
    str(lr)), listener_train_loss)
np.save('loss_acc_older/li_train_acc_{}_{}'.format(str(batchsize),
    str(lr)), listener_train_acc)
np.save('loss_acc_older/sp_train_loss_{}_{}'.format(str(batchsize),
    str(lr)), speaker_train_loss)
np.save('loss_acc_older/sp_train_acc_{}_{}'.format(str(batchsize),
    str(lr)), speaker_train_acc)
np.save('loss_acc_older/li_val_loss_{}_{}'.format(str(batchsize),
    str(lr)), listener_val_loss)
np.save('loss_acc_older/li_val_acc_{}_{}'.format(str(batchsize),
    str(lr)), listener_val_acc)
np.save('loss_acc_older/sp_val_loss_{}_{}'.format(str(batchsize),
    str(lr)), speaker_val_loss)
np.save('loss_acc_older/sp_val_acc_{}_{}'.format(str(batchsize),
    str(lr)), speaker_val_acc)
np.save('loss_acc_older/li_test_loss_{}_{}'.format(str(batchsize),
    str(lr)), listener_test_loss)
np.save('loss_acc_older/li_test_acc_{}_{}'.format(str(batchsize),
    str(lr)), listener_test_acc)
np.save('loss_acc_older/sp_test_loss_{}_{}'.format(str(batchsize),
    str(lr)), speaker_test_loss)
np.save('loss_acc_older/sp_test_acc_{}_{}'.format(str(batchsize),
    str(lr)), speaker_test_acc)

############ SAVE THE PRETRAINED MODEL ################
#
torch.save(listener.state_dict(), 'pretrained_sibling_models/{}_{}_{}_checkpoint_sibling_older_listener.pth'.format(str(batchsize),lr, str(epochs)))
torch.save(speaker.state_dict(), 'pretrained_sibling_models/{}_{}_{}_checkpoint_sibling_older_speaker.pth'.format(str(batchsize),lr,str(epochs)))

torch.save({
            'epoch': epochs,
            'model_state_dict': listener.state_dict(),
            'optimizer_state_dict': listener_optimizer.state_dict(),
            'loss': listener_train_loss
            }, 'pretrained_sibling_models/{}_{}_{}_checkpoint_sibling_older_listener.pth'.format(str(batchsize),lr,str(epochs)))

torch.save({
            'epoch': epoch,
            'model_state_dict': speaker.state_dict(),
            'optimizer_state_dict': speaker_optimizer.state_dict(),
            'loss': speaker_train_loss
            }, 'pretrained_sibling_models/{}_{}_{}_checkpoint_sibling_older_speaker.pth'.format(str(batchsize),lr,str(epochs)))
