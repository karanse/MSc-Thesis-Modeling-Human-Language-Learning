...
def curiosity(i, o):
    ...
    # i and o are torch tensors of shape [ntokens]
...
def curious_look_at_img(dict_words_boxes, ha_vggs_indices, img):
    ...
    object_guesses = nn.functional.softmax(listener(words, visual_input), dim=1)
    ...
    #, curiosity_values.data, subjective_novelty.data, plasticity.data, input_words
...
listener = curious_agent.Listener(n_objects, object_size, ntokens, wordemb_size,
        att_hidden_size).to(device)

# Makes the speaker part of the model:
speaker = curious_agent.Speaker(object_size, ntokens, att_hidden_size).to(device)
...
np.save('loss_acc/li_train_loss_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), listener_train_loss)
np.save('loss_acc/li_train_acc_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), listener_train_acc)
np.save('loss_acc/sp_train_loss_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), speaker_train_loss)
np.save('loss_acc/sp_train_acc_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), speaker_train_acc)
np.save('loss_acc/li_val_loss_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), listener_val_loss)
np.save('loss_acc/li_val_acc_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), listener_val_acc)
np.save('loss_acc/sp_val_loss_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), speaker_val_loss)
np.save('loss_acc/sp_val_acc_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), speaker_val_acc)
np.save('loss_acc/li_test_loss_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), listener_test_loss)
np.save('loss_acc/li_test_acc_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), listener_test_acc)
np.save('loss_acc/sp_test_loss_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), speaker_test_loss)
np.save('loss_acc/sp_test_acc_{}_{}_{}'.format(str(batchsize),
    str(lr), setting), speaker_test_acc)
